...
暫時不要因為寫好 exception ladder 就接 CRM、付款、客戶資料、正本覆寫或自動外發工具；它只證明你較能處理已知例外，不代表 workflow 已安全或 autonomous。若你未寫過完整 Skill，可先讀 [Skill 其實是一份工作合約](./4-23-你以為技能是一段指令其實它是一份工作合約.md)；想把重複錯變成可重跑檢查，可讀 [最小 regression pack](./4-10-人工智能工作流程每次一改就怕另一格壞.md)，需要另一個人或 AI 幫你核對時則讀 [設計獨立 review](./4-18-人工智能不能自己檢查自己.md)。
