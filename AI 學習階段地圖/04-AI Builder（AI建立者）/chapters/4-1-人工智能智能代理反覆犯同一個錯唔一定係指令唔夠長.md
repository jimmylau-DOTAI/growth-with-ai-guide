...
若要把例外處理寫成共同語言，可讀 [AI 一遇資料不足就繼續猜？Skill 要先寫好 exception ladder](./4-34-人工智能一遇資料不足就繼續猜.md)；要把已知錯收進固定檢查，可讀 [最小 regression pack](./4-10-人工智能工作流程每次一改就怕另一格壞.md)。
...
暫時不要把所有 prompt 都改成 code；不要把判斷、策略、品味或人際責任硬寫成假規則；也不要因為有 harness 就授予外部 action。想先設計一條可停、可 review 的 Agent loop，可讀 [先跑一條可停的 Agent loop](./4-25-先跑一條可停止的智能代理循環.md)；若你的工作仍混合了人手判斷與可重跑部分，讀 [唔係每條 SOP 都應該變成 AI Skill](./4-37-唔係每條標準作業程序都應該變成人工智能技能.md)。
