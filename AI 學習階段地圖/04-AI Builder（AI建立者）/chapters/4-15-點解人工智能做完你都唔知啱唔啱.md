...
完成後保留 state、change request、diff 和 review 結果。暫時不要把這種流程接到外部發送、production 寫入或未公開／客戶資料；先在可回看的 draft 工作證明它有用。下一步可看 [AI Agent 要有 loop，而不是一次性回答](./4-3-人工智能循環點樣先唔係排咗期嘅指令.md)，再把這張 state card 放進一條有 stop line 的工作 loop。
