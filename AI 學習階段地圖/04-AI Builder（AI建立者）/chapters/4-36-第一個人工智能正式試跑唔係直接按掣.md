...
想把例外狀態寫進 Skill，可讀 [AI 一遇資料不足就繼續猜？Skill 要先寫好 exception ladder](./4-34-人工智能一遇資料不足就繼續猜.md)。
...
暫時不要在第一次 live run 接 CRM、付款、客戶資料、外發、正本覆寫或不可逆部署；一次 human approval 亦不代表 workflow 已 production-ready。想把重複已知錯收進每次都會跑的檢查，可讀 [最小 regression pack](./4-10-人工智能工作流程每次一改就怕另一格壞.md)；想知道 output 是否真能收貨，可讀 [設計獨立 review](./4-18-人工智能不能自己檢查自己.md)。
