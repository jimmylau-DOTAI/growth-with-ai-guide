...
想把這些狀態寫成 Skill 語言，可讀 [AI 一遇資料不足就繼續猜？Skill 要先寫好 exception ladder](./4-34-人工智能一遇資料不足就繼續猜.md)。
...
暫時不要把所有創作結果寫成硬規則；不要因為有 gate 就跳過 human review；更不要把一次通過當成 production-ready。若你需要先判斷哪段 SOP 值得做成 Skill，可讀 [唔係每條 SOP 都應該變成 AI Skill](./4-37-唔係每條標準作業程序都應該變成人工智能技能.md)；要將已知錯做成更可重跑檢查，可讀 [最小 regression pack](./4-10-人工智能工作流程每次一改就怕另一格壞.md)。
