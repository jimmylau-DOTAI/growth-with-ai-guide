...
想把這些狀態寫得更完整，可讀 [AI 一遇資料不足就繼續猜？Skill 要先寫好 exception ladder](./4-34-人工智能一遇資料不足就繼續猜.md)。
...
暫時不要因為一次示範成功就叫它 autonomous；不要把 demo 畫面當成可讀工作狀態；也不要因為錄過一次便開放私人 chat、CRM、合約、付款、credentials 或 production setting。若你要把這份候選寫成正式工作合約，可讀 [Skill 唔係存低一段長 prompt](./4-6-技能唔係存低一段長指令.md)；若你仍未知道哪段工作應保留人手，讀 [唔係每條 SOP 都應該變成 AI Skill](./4-37-唔係每條標準作業程序都應該變成人工智能技能.md)。
